...
class Income(BaseModel, table=True):
    ...
    id: Optional[UUID] = Field(
        ...
        primary_key=True,
        sa_column_kwargs={"name": "IncomeID"}
    )
    ...
    amount: Optional[float] = Field(
        sa_column_kwargs={"name": "Amount"}
    )
    description: Optional[str] = Field(
        max_length=500,
        sa_column_kwargs={"name": "Description"}
    )
    income_date: Optional[datetime] = Field(
        sa_column_kwargs={"name": "IncomeDate"}
    )
